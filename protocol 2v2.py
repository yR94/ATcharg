...
try:
    ...
    ###############################################
       ...
       targz = int(np.mean(targz)*1000 )#set point dist at 200)
       # if targz is 0:
       #     continue
          # targz = np.mean(np.nonzero(zz))
       ...
       targx_norm = targx / norm
       targy_norm = targy / norm
       targz_norm = targz / norm
       ...
       epsilon_z = 200
       zoffste =150
       if targz-zoffste > epsilon_z:
           target_i = target_ref * transl(200 *targy_norm, -200 *targx_norm, 200 *targz_norm)
       elif  targz==0:
        target_i = target_ref * transl(0, 0, -targz_norm )
            ...
            target_i = target_ref*transl(0, 0,targz-zoffste)#*rotx(-theta_x)*roty(-theta_y)#(X=Ycam,Y=-Xcam,Z=Zcam)

       # target_i = target_ref*transl(targy,-targx,0)#*rotx(-theta_x)*roty(-theta_y)#(X=Ycam,Y=-Xcam,Z=Zcam)
       # target_i = target_ref*transl(-100,0,0)#*rotx(-theta_x)*roty(-theta_y)#(X=Ycam,Y=-Xcam,Z=Zcam)
       epsilon = norm
       if abs(targx)<2.5 and abs(targy)<2.5 and abs(targz-zoffste) < 1:
           epsilon = 1
           break
