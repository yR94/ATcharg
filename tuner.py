...
try:
    ...
    ###############################################
       ...
       if np.abs(targx) > epsilon_xy or np.abs(targy) > epsilon_xy:
            target_i = target_ref*transl(targy,-targx,0)#*rotx(-theta_x)*roty(-theta_y)#(X=Ycam,Y=-Xcam,Z=Zcam)
       else:
            start_pos = robot.Pose()
            target_i = target_ref*transl(0, 0, 300*targz_norm)#*rotx(-theta_x)*roty(-theta_y)#(X=Ycam,Y=-Xcam,Z=Zcam)
            robot.MoveL(target_i)
            end_pos = robot.Pose()
            print(Pose_2_TxyzRxyz(start_pos), "          ",Pose_2_TxyzRxyz(end_pos))
            break
